...
- [Skills](#skills-copy-into-claude-skills)
- [Rules](#rules-copy-into-claude-rules)
...
mkdir -p .claude/skills/db-patterns
...
# Create all config files using the templates below
touch CLAUDE.md
touch REVIEW.md
touch .env.example
touch .gitignore
...
touch .claude/agents/verify-app.md
...
touch .claude/skills/db-patterns/SKILL.md
touch docs/requirements.md
touch docs/architecture.md
touch docs/plan.md
touch tasks/lessons.md
touch tasks/current-task.md
...
This is the complete folder and file structure for every project.
Every file has a specific purpose explained below.

```
your-project/
│
├── CLAUDE.md                          ← Brain loaded at start of every session
│                                         Under 200 lines. Global rules and commands.
│                                         Updated after every mistake — permanent memory.
│
├── REVIEW.md                          ← Code review rules separate from CLAUDE.md
│                                         Used by the code-reviewer agent.
│                                         Add project-specific review checklist here.
│
├── .env.example                       ← All required environment variables documented
│                                         No real values — just keys with descriptions.
│                                         Every new developer copies this to .env
│
├── .gitignore                         ← Standard ignores for your language + .env
│
├── .claude/
│   │
│   ├── settings.json                  ← Tool permissions, hooks, model config
│   │                                     Check into git — whole team benefits.
│   │                                     Hooks auto-format code and run tests.
│   │
...
│   │   │
│   │   ├── plan.md                    → /plan
│   │   │                                 Reads requirements + codebase.
│   │   │                                 Writes 4-phase plan to docs/plan.md.
│   │   │                                 Does NOT implement.
│   │   │
│   │   ├── implement.md               → /implement
│   │   │                                 Reads docs/plan.md.
│   │   │                                 Executes approved plan with TDD.
│   │   │                                 Stops after each phase.
│   │   │
│   │   ├── review.md                  → /review
...
│   │   │
│   │   ├── test.md                    → /test
│   │   │                                 Generates tests for new behaviour.
│   │   │                                 Follows existing test structure.
│   │   │                                 Runs full suite and reports coverage.
│   │   │
│   │   ├── fix-issue.md               → /fix-issue [description]
│   │   │                                 Takes $ARGUMENTS as issue description.
│   │   │                                 Finds root cause, writes failing test,
│   │   │                                 fixes implementation, adds to lessons.md.
│   │   │
│   │   ├── checkpoint.md              → /checkpoint
│   │   │                                 Saves current state to tasks/current-task.md.
│   │   │                                 Lists modified files, test status, phase.
│   │   │
│   │   └── learn.md                   → /learn
│   │                                     Extracts patterns from current session.
│   │                                     Writes to tasks/lessons.md.
│   │                                     Suggests CLAUDE.md additions.
│   │
│   ├── agents/                        ← Specialist subagents with isolated context
│   │   │                                 Each runs in its own fresh context window.
│   │   │                                 Called with: "Use the X subagent on Y"
│   │   │
│   │   ├── planner.md                 ← Creates phased implementation plans
│   │   │                                 Model: opus · effort: high
│   │   │                                 Used during Step 2 of new product workflow.
│   │   │
│   │   ├── code-reviewer.md           ← Reviews code as principal engineer
│   │   │                                 Model: opus · effort: high
│   │   │                                 Called after /review for Critical+High fixes.
│   │   │
│   │   ├── security-reviewer.md       ← Reviews for security vulnerabilities
│   │   │                                 Model: opus · effort: high
│   │   │                                 Run on src/ after every product phase.
│   │   │                                 Run on changed files for feature changes.
│   │   │
│   │   ├── test-writer.md             ← Writes comprehensive tests for any module
│   │   │                                 Model: sonnet
│   │   │                                 Used when /test needs deeper coverage.
│   │   │
│   │   └── verify-app.md              ← Verifies full system readiness
│   │                                     Model: sonnet
│   │                                     Run after every phase checkpoint.
│   │                                     Reports READY or NOT READY.
│   │
│   ├── skills/                        ← Knowledge loaded on demand, not every session
│   │   │                                 Keeps context lean — only loads when needed.
│   │   │                                 Each skill is a folder with a SKILL.md inside.
│   │   │
│   │   ├── project-conventions/
│   │   │   └── SKILL.md               ← Your project's specific patterns and idioms
│   │   │                                 Naming conventions, file organisation rules,
│   │   │                                 patterns that exist in your codebase.
│   │   │                                 Update this as your codebase evolves.
│   │   │
│   │   ├── api-conventions/
│   │   │   └── SKILL.md               ← Your API design rules
│   │   │                                 REST/GraphQL/gRPC style, error formats,
│   │   │                                 status codes, auth headers, versioning.
│   │   │
│   │   └── db-patterns/
│   │       └── SKILL.md               ← Your database conventions
│   │                                     Migration tool, query patterns,
│   │                                     soft delete policy, indexing rules.
│   │
│   └── rules/                         ← Always-follow guidelines loaded automatically
│       │                                 Referenced from CLAUDE.md via @import.
│       │                                 Split large CLAUDE.md instructions here.
│       │
│       ├── coding-style.md            ← Language-specific style rules
│       │                                 Naming, formatting, import order,
│       │                                 patterns preferred in this project.
│       │
│       ├── git-workflow.md            ← Git rules
│       │                                 Branch naming, commit message format,
│       │                                 PR process, what never goes into git.
│       │
│       └── testing.md                 ← Testing rules
│                                         Framework, file naming, coverage threshold,
│                                         what must always be tested.
│
├── docs/
│   │
│   ├── requirements.md                ← Generated in Step 1 of new product workflow
│   │                                     Functional + non-functional requirements.
│   │                                     Out of scope list. Success metrics.
│   │                                     Source of truth for what the product does.
│   │
│   ├── architecture.md                ← Generated alongside requirements
│   │                                     Module breakdown with phase labels.
│   │                                     Dependency diagram. Tech decisions.
│   │                                     Updated when architecture changes.
│   │
│   ├── plan.md                        ← Active task plan — the most used file
│   │                                     Written by /plan command.
│   │                                     YOU annotate this directly with > NOTE: lines.
│   │                                     Guards: "address all notes, don't implement yet"
│   │                                     Overwritten each time /plan runs.
│   │
│   └── decisions/                     ← Architecture Decision Records (ADRs)
│       └── adr-001-example.md            One file per major technical decision.
│                                         Format: context, options, decision, consequences.
│                                         Never delete — mark old ones as superseded.
│
├── tasks/
│   │
│   ├── lessons.md                     ← Self-improvement log — most important file
│   │                                     Updated by /learn after every session.
│   │                                     Format: Mistake + Rule that prevents it.
│   │                                     Referenced by CLAUDE.md via @import.
│   │                                     This is how Claude gets better permanently.
│   │
│   └── current-task.md                ← Active session state
│                                         Written by /checkpoint command.
│                                         Modified files, test status, current phase.
│                                         Paste this into new session after /clear.
│
├── src/                               ← Your actual application code
│   └── [organised by your language and framework conventions]
│
├── tests/                             ← Your test files
│   └── [organised to mirror src/ structure]
│
└── .mcp.json                          ← MCP server configurations
                                          Checked into git — whole team gets same tools.
                                          GitHub, Slack, database tools, etc.
```

### What each file type does — summary

| File / Folder | When it is created | Who writes it | What it controls |
|---|---|---|---|
| `CLAUDE.md` | Project setup | You (from template) | Global behaviour every session |
| `REVIEW.md` | Project setup | You | Code review checklist |
| `.claude/settings.json` | Project setup | You (from template) | Permissions, hooks, model |
| `.claude/commands/*.md` | Project setup | You (from templates) | Slash commands you run |
| `.claude/agents/*.md` | Project setup | You (from templates) | Specialist subagents |
| `.claude/skills/*/SKILL.md` | Project setup | You | Domain knowledge on demand |
| `.claude/rules/*.md` | Project setup | You | Always-on rules referenced by CLAUDE.md |
| `docs/requirements.md` | Step 1 brainstorm | Claude writes, you review | What the product does |
| `docs/architecture.md` | Step 1 brainstorm | Claude writes, you review | How it is structured |
| `docs/plan.md` | Every /plan run | Claude writes, you annotate | Active task plan |
| `docs/decisions/*.md` | When decisions are made | You or Claude | Why decisions were made |
| `tasks/lessons.md` | Every /learn run | Claude writes | Permanent mistake log |
| `tasks/current-task.md` | Every /checkpoint run | Claude writes | Current session state |
...
Skills are knowledge files loaded on demand — not every session. This keeps your context lean. Each skill is a folder containing a `SKILL.md` file. Copy these three starter skills and fill in your project's specifics.
...
description: Coding patterns, naming conventions, and idioms specific to this project
---
# Project conventions

## Naming conventions
[Describe how files, functions, classes, variables are named in your project]
Example: All service files are named [entity].service.[ext]
Example: All test files are named [entity].test.[ext] in a tests/ folder mirroring src/

## File organisation
[Describe how code is organised in your project]
Example: Each module lives in src/modules/[name]/ with service, repository, types, errors

## Patterns used consistently
[List patterns that appear repeatedly in your codebase]
Example: Repository pattern for all database access
Example: Dependency injection via constructor — never instantiate inside functions
Example: Named exports only — no default exports

## What to avoid
[List anti-patterns that have been explicitly rejected]
Example: No raw SQL — always use the ORM
Example: No console.log — use the structured logger at src/shared/logging/

## Gotchas
[List mistakes Claude has made before in this project]
[Add to this section whenever /learn identifies a recurring mistake]
```
...
description: API design rules and patterns for this project
---
# API conventions

## Style
[REST / GraphQL / gRPC / tRPC — your choice and why]

## Authentication
[How auth is passed — Bearer token, cookie, API key, etc]

## Error response format
[Your standard error shape]
Example:
{
  "error": {
...
## Status codes used
[Which HTTP status codes you use and what they mean in your system]
...
## Request validation
[Where validation happens — at the route handler, middleware, service layer]

## Versioning
[How API versions are managed — URL prefix, headers, etc]

## Logging
[What every request must log — method, path, status, duration, user id]
```

### `db-patterns/SKILL.md`

```markdown
---
name: db-patterns
description: Database conventions and patterns for this project
---
# Database patterns

## Migration tool
[Alembic / Flyway / Drizzle / Prisma / golang-migrate / Liquibase / etc]
Migration command: [command to run migrations]
Rollback command: [command to rollback one migration]
Create migration: [command to create new migration file]
...
- Never use raw SQL — always use the ORM or query builder
- Index every foreign key column
- Add created_at and updated_at to every table
- Soft delete with deleted_at — never hard delete user-generated data
- Never rename a column in one migration — add new, copy, drop old in three steps

## Query patterns
[Describe how queries are written in your project]
Example: All queries go through repository classes
Example: Use query builders not string concatenation for dynamic queries

## Connection handling
[How connections are managed — pool settings, timeout config]

## Gotchas
[Database-specific issues discovered during development]
[Add to this section whenever /learn identifies a database mistake]
```

---

## Rules — Copy into `.claude/rules/`

Rules are always-on guidelines referenced by `CLAUDE.md` via `@import`. They load every session without consuming tokens in the visible context. Copy these three starter rule files and fill in your specifics.
...
## File naming
[Your convention — kebab-case, snake_case, PascalCase, etc]

## Function and method naming
[Your convention — camelCase, snake_case, verb prefix requirements, etc]

## Class and type naming
[Your convention — PascalCase, interfaces prefixed with I, etc]

## Variable naming
[Your convention — descriptive names, no abbreviations, etc]

## Import organisation
[Order of imports — stdlib, third party, internal — with blank lines between]

## Maximum lengths
- Function body: 30 lines — split into smaller functions if longer
- File length: 200 lines — split into modules if longer
- Line length: [your limit — 88, 100, 120 characters]

## Comments
- Every public function and class needs a docstring or JSDoc comment
- No commented-out code in commits — delete it
- TODO comments must include a ticket number: TODO(#123): description
- Explain WHY not WHAT — the code shows what, comments explain why

## Specific patterns preferred in this project
[Add your project's specific style rules here]
```
...
- tokens now rotate on every use
- old tokens are invalidated immediately after use
- adds refresh_tokens table to store valid tokens
...
- Commit at least once per completed task — do not accumulate changes
...
- IDE config files (.idea/, .vscode/ personal settings)
```
...
## Framework
[pytest / jest / go test / JUnit / RSpec / PHPUnit / etc]

## File naming convention
[test_[name].py / [name].test.ts / [name]_test.go / [Name]Test.java]

## Folder structure
[Mirror src/ in tests/ / co-locate with source / separate tests/ folder]

## Coverage threshold
Minimum 80% line coverage required
CI pipeline fails below this threshold

## What must always be tested
- Every public function — happy path and every failure path
- Every API endpoint — success, auth failure, validation failure, not found
- Every database operation — create, read, update, delete
- Every external service call — success and failure
- Edge cases: empty input, null/nil/None, maximum values, zero

## What does not need tests
- Private helper functions used only internally
- Simple getters and setters with no logic
- Generated code

## Mocking rules
[How mocks are used — mock external services, not internal modules]
[Which mock library — unittest.mock / jest.mock / testify / Mockito]

## Test data
[Where fixtures live, how test databases are managed, how to reset state]

## Rules
- Write the failing test before the implementation (TDD)
- Never delete existing tests to make the count green
- Never skip or mark tests as pending without a ticket number
- Fix implementation when tests fail — do not modify tests to pass
